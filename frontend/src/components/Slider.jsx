    ...
    <div className='slider'>
      ...
      <div className="slider-content">
        <div className="convo-filters">
          <button onClick = {()=>{setButtonCount(1),setSliderComponent(1),setUser(null),setSelectedGroup(null)}} className= {`convo-button ${sliderComponent === 1 ? "active" : ""}`}>
          ...
          <button onClick = {()=>{setButtonCount(2),setSliderComponent(2),setUser(null),setSelectedGroup(null)}} className= {`convo-button ${sliderComponent === 2 ? "active" : ""}`}>
          ...
          <button onClick = {()=>{setButtonCount(3),setSliderComponent(3),setUser(null),setSelectedGroup(null)}} className= {`convo-button ${sliderComponent === 3 ? "active" : ""}`}>
          ...
          <button onClick = {()=>{setButtonCount(4),setSliderComponent(4),setUser(null),setSelectedGroup(null)}} className= {`convo-button ${sliderComponent === 4 ? "active" : ""}`}>
          ...
          <button onClick = {()=>{setButtonCount(5),setSliderComponent(5),setUser(null),setSelectedGroup(null)}} className= {`convo-button ${sliderComponent === 5 ? "active" : ""}`}>
        ...
        <div className="convo-friends">
          <div className="convo-users">
              ...
              <svg onClick={() => {setOpenGroup(true),setUser(null)}} style={{ cursor: "pointer" }} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-5">
              <path fill-rule="evenodd" d="M12 3.75a.75.75 0 0 1 .75.75v6.75h6.75a.75.75 0 0 1 0 1.5h-6.75v6.75a.75.75 0 0 1-1.5 0v-6.75H4.5a.75.75 0 0 1 0-1.5h6.75V4.5a.75.75 0 0 1 .75-.75Z" clip-rule="evenodd" />
              </svg>
              </h2>
              {users.filter((x)=>x.is_friend || x.has_dm).map((x) => {
                return (
                  <li key={x.id} className={`convo-user ${user == x.id ? "active" : ""}`} onClick={() => { setUser(x.id),setButtonCount(0),setSliderComponent(0),setSelectedGroup(null)}}>
              ...
              {groups.map((x)=>{
                return (
                  <li key={x.id} className={`convo-user ${selectedGroup == x.id ? "active" : ""}`} onClick={() => { setUser(null),setButtonCount(0),setSliderComponent(0),setSelectedGroup(x.id)}}>
                    <div className="user-profile">
                      <div className="avatar-wrapper">
                        <img className="w-[40px] h-[40px] rounded-[100%]" src={x.image}/>
                      </div>
